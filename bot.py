...
# ── CONFIGURATION (GitHub Secrets / Environment Variables မှ ရယူမည်) ────[span_2](start_span)[span_2](end_span)
BOT_TOKEN = os.getenv("BOT_TOKEN")

# သုံးစွဲခွင့်ပြုမည့် အကောင့် ID (၃) ခု[span_3](start_span)[span_3](end_span)
DEFAULT_ADMIN_IDS = {8776761081, 8477333962, 8635066797}
admin_env = os.getenv("ADMIN_ID", "")
if admin_env:
    try:
        EXTRA_IDS = {int(i.strip()) for i in admin_env.split(",") if i.strip().isdigit()}
        ADMIN_IDS = DEFAULT_ADMIN_IDS.union(EXTRA_IDS)
    except Exception:
...
if not BOT_TOKEN or not ADMIN_IDS:
    raise ValueError("❌ BOT_TOKEN သို့မဟုတ် ADMIN_ID ကို GitHub Secrets တွင် မှန်ကန်စွာ ထည့်သွင်းပေးပါ။")[span_4](start_span)[span_4](end_span)

BATCH_SIZE = 500        # Telegram Bot အတွက် သင့်တော်သော Batch Size[span_5](start_span)[span_5](end_span)
MAX_CONCURRENT = 3000     # သင့်တော်သော Concurrent Level[span_6](start_span)[span_6](end_span)
CONNECTION_LIMIT = 1500[span_7](start_span)[span_7](end_span)
TIMEOUT = 25[span_8](start_span)[span_8](end_span)

# ── GLOBALS ──────────────────────────────────────────────────────────────[span_9](start_span)[span_9](end_span)
_connector = None[span_10](start_span)[span_10](end_span)
_ocr = None[span_11](start_span)[span_11](end_span)
DIGITS = list(string.digits)[span_12](start_span)[span_12](end_span)
LOWERCASE_CHARS = list(string.ascii_lowercase)[span_13](start_span)[span_13](end_span)
MIXED_CHARS = list(string.ascii_lowercase + string.digits)[span_14](start_span)[span_14](end_span)

# User တစ်ယောက်ချင်းစီ၏ Scan State များကို မှတ်ထားရန် Dict[span_15](start_span)[span_15](end_span)
user_scans = {}[span_16](start_span)[span_16](end_span)

# ── OCR INITIALIZATION ───────────────────────────────────────────────────[span_17](start_span)[span_17](end_span)
def init_ocr():
    global _ocr
    if _ocr is None:
...
# ── HELPER FUNCTIONS ─────────────────────────────────────────────────────[span_18](start_span)[span_18](end_span)
...
# ── GENERATORS ──────────────────────────────────────────────────────────[span_19](start_span)[span_19](end_span)
...
# ── CAPTCHA & RUIJIE API ─────────────────────────────────────────────────[span_20](start_span)[span_20](end_span)
...
# ── TELEGRAM HANDLERS ────────────────────────────────────────────────────[span_21](start_span)[span_21](end_span)

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    if not is_admin(user_id):
        await update.message.reply_text("❌ ခွင့်ပြုချက်မရှိပါ။ ခွင့်ပြုထားသော အကောင့်များသာ အသုံးပြုနိုင်ပါသည်။")
        return
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        await query.answer("❌ ခွင့်ပြုချက်မရှိပါ။ ခွင့်ပြုထားသော အကောင့်များသာ အသုံးပြုနိုင်ပါသည်။", show_alert=True)
        return
...
# ── BACKGROUND SCAN TASK ────────────────────────────────────────────────[span_22](start_span)[span_22](end_span)
...
# ── MAIN BOT RUNNER ──────────────────────────────────────────────────────[span_23](start_span)[span_23](end_span)
...
def main():
    ...
    print("🤖 Telegram Bot Starting (Multi-User Security Active)...")
    app.run_polling()
